...
| 正式实时决策 | 所需完整历史从本地读取、缺口按上述日线路由补齐；事件价独立使用 Alpaca IEX |
| 分钟历史 | 仅 Alpaca |
...
事件时点校验完整历史并独立调用 IEX API 获取任务候选池的精确快照。
...
- 所有运行实例由一个可唤醒的中央调度线程管理；普通等待状态不占用事件工作线程，也不
  ...
  一次决策并调用 Alpaca IEX。该正式事件取数不受行情总览自动更新开关影响，且只处理任务
  候选池及代码策略声明的辅助标的。
- 任务启动和每个正式事件都会按策略实际回看长度，核对候选池及辅助标的在事件日前所需的
  每一个已完成交易日。非总览标的同样会按需补数；补数后仍缺日、存在未完成历史 K 线，或
  任一非加密候选没有有效事件行情时，本次正式决策失败关闭，不再用部分候选池重新排名。
