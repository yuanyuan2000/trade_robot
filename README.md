...
| 模块 | 当前能力 |
| --- | --- |
| 行情总览 | 展示自选标的、涨跌幅和更新时间；支持两列收藏指标、排序、拖拽和移除 |
...
- “指标列 1/2”可各选择一个收藏指标并展示其最新日线值；两列都支持升降序，选择保存在
...
- 每个运行实例使用独立任务线程，行情由共享 IEX 数据服务获取；免费 Alpaca 计划固定使用
  `feed=iex`，不使用 SIP。
...
| `/api/realtime/tasks/*` | 实时决策任务 CRUD、启动/终止、校验和事件 SSE |
...
实时决策专项检查还覆盖开盘/收盘数据边界、策略计算复用、邮件密钥脱敏和一分钟并发限频。
