...
- 行情总览的标准指标统一按前复权日线计算，只读取已经落库的公司行动，不会因打开总览
  触发外部公司行动请求。若最新日线尚未收线，单元格标记“盘中”，悬停可查看价格口径、
  数据日和截至时间。
...
- 面板支持搜索、状态筛选、仅看候选池、动态指标列排序和行级计算明细。代码策略的列由
  策略代码定义：七星展示长期年化趋势、R²、短动量、可选量比和评分；ATR 展示 N 日价格
  位移、ATR 和评分；WTME 展示 Rw、Aw 和评分。列标题问号会按任务当前生效参数说明回看天数、计算口径、用途和关键过滤阈值。
...
| 场景 | 指标/策略输入 | 未收线处理 |
| --- | --- | --- |
| 行情总览标准指标 | 本地已存公司行动前复权后的日线 | 纳入当前暂定日线并标记盘中 |
