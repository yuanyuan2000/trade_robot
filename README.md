...
| 指标 | MA、EMA、Wilder ATR、相对 ATR 动量评分；按“标的 + K线周期”保存配置 |
...
- 美股/ETF 可选择“全复权”“仅拆股复权”或“原始价格”；页面列出当前范围内的拆股、
  ...
  保持原样并提示，避免重复复权。
- 详情 K 线默认使用全复权。MA、EMA、ATR、相对 ATR、WTME 等指标始终由服务端基于
...
- 非代码策略支持 single、distribution、competition；竞争买入本身即为策略规则，额外的
  ...
  OHLCV、MA、EMA、Wilder ATR、相对 ATR、WTME、急跌过滤及布尔/比较运算。
...
| `/api/backtest/strategies/*` | 策略 CRUD、验证、代码策略目录和运行历史 |
