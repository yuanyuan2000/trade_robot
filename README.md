...
- 总览中的“自动更新”开关是服务端唯一的行情总览更新协调器：应用打开时更新一次，开启后
  每 5 分钟更新一次；实时决策详情中的同名开关与这里共享同一状态。
...
服务端只有一个行情总览外部数据更新协调器。其开关状态保存在 `system_settings`，5 分钟
更新沿用同一范围和规则；单个标的失败不会中断其他标的，后续轮次可再次尝试。行情页面
每 60 秒只重读一次内部数据库，让总览和已打开的 K 线及时看到服务端同步结果，不会因此
额外请求外部行情。关闭这个开关只停止行情总览的周期更新，不会阻止实时决策任务在正式
事件时点独立调用 IEX API 获取任务候选池的精确快照。
...
面板禁止外部取数、关闭总览更新时正式事件仍独立取数、非代码三种模式、七星任务幂等、
邮件密钥脱敏和一分钟并发限频。
