...
| `REALTIME_MAX_WORKERS` | `4` | 实时决策任务线程上限，范围 1～8 |
...
- 每个运行实例使用独立任务线程；只在 OPEN、CLOSE 或策略指定的 `HH:MM` 事件到达时调用
  Alpaca IEX，没有 15 秒或每分钟的后台 IEX 轮询。该正式事件取数不受行情总览自动更新
  开关影响，且只处理任务候选池及代码策略声明的辅助标的。
