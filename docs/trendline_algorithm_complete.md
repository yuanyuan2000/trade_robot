...
是否纳入美股休市日由标的设置或 `show_non_us_market_days` 请求参数决定（旧参数
`show_weekend_data` 仅作兼容）。关闭时，先按保存的 XNYS 日历删除周末及节假日，再执行
周期聚合；开启时保留标的原始日历中的全部 K 线。
...
```text
算法版本、标的、period、limit、是否显示美股休市日、窗口位置、总数据量、完整窗口指纹
```
